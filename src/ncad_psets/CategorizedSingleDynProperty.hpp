...
public:
    ...
    /*mode= 0: свойство отображается для заполнения у всех текущих 
    и создаваемых объектов разрешенных классов;
      mode= 1: свойство отображается только у объектов, имеющих значение 
    (только для импорта внешних данных, например, ГИС)*/
    int show_mode = 0;
};
#endif
